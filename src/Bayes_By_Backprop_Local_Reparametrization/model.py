...
class BayesLinear_local_reparam(nn.Module):
    ...
    def __init__(self, n_in, n_out, prior_sig):
        ...
        self.W_mu = nn.Parameter(
            torch.Tensor(self.n_in, self.n_out).uniform_(-0.1, 0.1)
        )
    ...
    def forward(self, X, sample=False):
        ...
        if not self.training and not sample:  # This is just a placeholder function
            ...
            eps_W = Variable(
                self.W_mu.data.new(act_W_std.size()).normal_(mean=0, std=1)
            )
            ...
            kld = KLD_cost(
                mu_p=0, sig_p=self.prior_sig, mu_q=self.W_mu, sig_q=std_w
        ...
        for key in state_dict.keys():
            ...
            if layer_name != previous_layer_name:
                ...
                for i in range(Nsamples):
                    ...
                    KL_W += isotropic_gauss_loglike(
                        W, W_mu, std_w, do_sum=False
                    ) - self.model.prior_instance.loglike(W, do_sum=False)
                    KL_b += isotropic_gauss_loglike(
                        b, b_mu, std_b, do_sum=False
    ...
    def mask_model(self, Nsamples=0, thresh=0):
        ...
        for key in state_dict.keys():
            ...
            if layer_name != previous_layer_name:
                ...
                state_dict[layer_name + ".W_p"][
                    1 - mask_dict[layer_name + ".W"]
                ] = -1000

                state_dict[layer_name + ".b_mu"][1 - mask_dict[layer_name + ".b"]] = 0
                state_dict[layer_name + ".b_p"][
                    1 - mask_dict[layer_name + ".b"]
                ] = -1000
