...
def layer_act_hessian_recurse(prev_hessian, prev_weights, layer_pre_acts):
    ...
    D = prev_weights.data.new(batch_size, newside, newside).fill_(
        0
    )  # is just 0 for a piecewise linear
    #     D[:, I] = dd_act(layer_pre_acts) * act_grads

    Hl = torch.bmm(
        torch.t(prev_weights).unsqueeze(0).expand([batch_size, -1, -1]), prev_hessian
    )
...
def chol_scale_invert_kron_factor(factor, prior_scale, data_scale, upper=False):
    scaled_factor = data_scale * factor + prior_scale * torch.eye(factor.shape[0]).type(
        factor.type()
    )
