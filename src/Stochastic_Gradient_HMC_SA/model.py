...
class BNN_cat(BaseNet):  # for categorical distributions
    ...
    def create_opt(self):
        ...
        self.optimizer = H_SA_SGHMC(
            params=self.model.parameters(), lr=self.lr, base_C=0.05, gauss_sig=0.1
        )  # this last parameter does nothing
    ...
    def fit(self, x, y, burn_in=False, resample_momentum=False, resample_prior=False):
        ...
        loss = (
            loss * self.N_train
        )  # We use mean because we treat as an estimation of whole dataset
        loss.backward()

        # Gradient buffer to allow for dynamic clipping and prevent explosions
        if len(self.grad_buff) > 1000:
            self.max_grad = np.mean(self.grad_buff) + self.grad_std_mul * np.std(
                self.grad_buff
            )
            self.grad_buff.pop(0)
        # Clipping to prevent explosions
        self.grad_buff.append(
            nn.utils.clip_grad_norm_(
                parameters=self.model.parameters(), max_norm=self.max_grad, norm_type=2
            ).cpu()
        )
    ...
    def save_sampled_net(self, max_samples):
        ...
        cprint(
            "c",
            " saving weight samples %d/%d"
            % (len(self.weight_set_samples), max_samples),
        )
        return None
